...
from internal.shortener.store import cluster_state, create_short_link, resolve_short_link
...
def text_response(handler: BaseHTTPRequestHandler, status: int, body: bytes) -> None:
    handler.send_response(status)
        ...
        if parsed.path == "/":
            json_response(
                ...
                200,
                {
                    ...
                    "endpoints": ["/health", "/shorten", "/resolve?code=...", "/state"],
                },
            )
            return
        if parsed.path == "/health":
            text_response(self, 200, b"ok")
            return
